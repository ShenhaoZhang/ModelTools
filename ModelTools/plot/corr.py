...
def scatter(data:Union[pd.DataFrame,alt.Chart], 
            x:str, 
            y:str, 
            add_lm:bool = False, 
            add_vline = None, 
            add_hline = None, 
            add_other_line:list = None, 
            point_mark:dict = {}, 
            lm_mark:dict = {},
        ):
    ...
    point = base_chart.mark_circle(**point_mark).encode(
    ...
    if add_lm is True:
        reg_line = base_chart.transform_regression(x,y).mark_line(**lm_mark).encode(
            alt.X(x),
            alt.Y(y)
        )
        plot += reg_line
